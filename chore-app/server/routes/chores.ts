...
router.post('/', (req, res) => {
  const { title, description, assignee_id, recurrence = 'none', recurrence_day, start_date, end_date } = req.body;
  if (!title) return res.status(400).json({ error: 'title is required' });
  if (!start_date) return res.status(400).json({ error: 'start_date is required' });
  const result = db.prepare(`
    INSERT INTO chores (title, description, assignee_id, recurrence, recurrence_day, start_date, end_date)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `).run(title, description ?? null, assignee_id ?? null, recurrence, recurrence_day ?? null, start_date, end_date ?? null);
...
router.put('/:id', (req, res) => {
  ...
  const { title, description, assignee_id, recurrence, recurrence_day, start_date, end_date } = req.body;
  db.prepare(`
    UPDATE chores SET
      title = ?, description = ?, assignee_id = ?,
      recurrence = ?, recurrence_day = ?, start_date = ?, end_date = ?
    WHERE id = ?
  `).run(
